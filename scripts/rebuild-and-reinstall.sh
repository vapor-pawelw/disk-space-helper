...
SCHEME="${DISK_SPACE_HELPER_SCHEME:-DiskSpaceHelper}"
CONFIGURATION="${DISK_SPACE_HELPER_CONFIGURATION:-Debug}"
APP_NAME="${DISK_SPACE_HELPER_APP_NAME:-DiskSpaceHelper}"
PROJECT="${DISK_SPACE_HELPER_PROJECT:-DiskSpaceHelper.xcodeproj}"
...
main() {
  ...
  if ! open -n "$app_path"; then
    ...
    "$binary_path" >/tmp/disk-space-helper-relaunch.log 2>&1 &
  fi
