...
DEFAULT_RELEASE_REPO="vapor-pawelw/disk-space-helper"
...
verify_release_asset() {
  ...
  if ! gh release view "$tag" --repo "$owner_repo" --json assets --jq '.assets[].name' | grep -qx "DiskSpaceHelper.dmg"; then
    echo "Release exists but DiskSpaceHelper.dmg was not found on $tag." >&2
    return 1
  fi

  if ! gh release view "$tag" --repo "$owner_repo" --json assets --jq '.assets[].name' | grep -qx "DiskSpaceHelper.zip"; then
    echo "Release exists but DiskSpaceHelper.zip was not found on $tag." >&2
    return 1
  fi
}
...
read_homebrew_cask() {
  local tap_repo="$1"
  gh api -H "Accept: application/vnd.github.raw" "repos/${tap_repo}/contents/Casks/disk-space-helper.rb"
}
...
main() {
  ...
  local release_repo
  release_repo="${DISK_SPACE_HELPER_RELEASE_REPO:-$DEFAULT_RELEASE_REPO}"

  local tap_repo
  tap_repo="${DISK_SPACE_HELPER_HOMEBREW_TAP_REPO:-$DEFAULT_HOMEBREW_TAP_REPO}"
  ...
  echo "- Verify GitHub release in ${release_repo} contains DiskSpaceHelper.dmg and DiskSpaceHelper.zip"
  ...
  expected_cask_url="https://github.com/${release_repo}/releases/download/${tag}/DiskSpaceHelper.dmg"
  ...
  if [[ -z "$cask_sha" ]]; then
    ...
    echo "Check manually: https://github.com/${tap_repo}/blob/main/Casks/disk-space-helper.rb" >&2
    exit 1
  fi
  ...
  echo "- Homebrew cask: https://github.com/${tap_repo}/blob/main/Casks/disk-space-helper.rb"
